...
router.get(
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      const { scans, total } = await scanQueries.findByUserId(
        req.userId!,
        { projectId, limit, offset, sort }
      );
  ...
  async (req: AuthenticatedRequest, res: Response, next: NextFunction) => {
    try {
      ...
      if (scan.status !== "completed") {
        throw new ValidationError(
          "Scan is not yet completed. Cannot generate report."
        );
      }

      // Build a ScanResult-compatible object from the stored scan data
